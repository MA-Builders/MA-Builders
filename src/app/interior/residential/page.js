...
export default function ConCommercial() {
  ...
  // ✅ Fetch only Interior → Residential projects
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          const filtered = data.filter(
            (p) =>
              p.category === "Interior" && p.subCategory === "Residential"
          );
      ...
      <div className="container-fluid py-5">
        <div className="container py-5">
          ...
          {loading ? (
            ...
            <p className="text-center text-gradient">No Interior projects found.</p>
          ) : (
